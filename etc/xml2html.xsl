...
<!--
    ...
    August 18, 2005 - first cut
    June   12, 2026   - tweaking for the Reader
...
-->
			...
			<body style='margin:5%;'>
				...
				<xsl:if test='/TEI/text/body/div'>
					...
					<xsl:for-each select="/TEI/text/*/div">
						...
						<a><xsl:attribute name="href">#<xsl:value-of select='./@xml:idno' /></xsl:attribute><xsl:value-of select='./head' /></a><br />
						</xsl:if>
					</xsl:for-each>
					</p>
				</xsl:if>
	...
	<xsl:template match="div">
		...
		<h2><a><xsl:attribute name="name"><xsl:value-of select='./@xml:idno' /></xsl:attribute><xsl:value-of select='./head' /></a></h2>
		<xsl:apply-templates />
	</xsl:template>
